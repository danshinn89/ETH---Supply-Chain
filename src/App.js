...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.itemMananger =  new this.web3.eth.Contract(
  ...
  listenToPaymentEvent = () => {
    let self = this;
    this.itemMananger.events.SupplyChainStep().on("data" , async function(evt) {
      console.log(evt);
      let itemObj = await self.itemMananger.methods.items(evt.returnValues._itemIndex).call();
      alert("Item " + itemObj._identifier + " was paid for, Please deliver");
    });
  }
  ...
  handleSubmit = async() => {
    ...
    let result = await this.itemMananger.methods.createItem(itemName, cost).send({from: this.accounts[0]});
    console.log(result);
